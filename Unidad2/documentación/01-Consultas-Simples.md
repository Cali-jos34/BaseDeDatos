...
-- Proyecci�n
--Seleccionar o mastrar el n�mero de empleado, su primer nombre, su cargo, ciudad y pais.

Select EmployeeID, FirstName, Title, City, Country
from Employees;

--Alias de columna 
--En base a la consulta anterior, visualizar el EmployeeID como n�mero empleado, FirstName como primer nombre, title como cargo, City como ciudad, Country como pais
...
--seleccionar las fechas de orden, a�o , mes y d�a
...
--Mostrar el nombre completo del empleado, su numero de empleado, fecha de nacimiento, ciudad y fecha de contrataci�n
--y esta debe ser de aquellos que fueron contratados despu�s de 1993, los resultados en sus encabezados deben ser mostrados
--en espa�ol.
select *from Employees
...
--Seleccionar clientes de M�xico o Estados Unidos
...
--Seleccionar los Pedidos con flete de peso (Freight) mayor a 100 y enviados a Francia o Espa�a
...
--Seleccionar los productos con precio entre $10 y $50 que NO est�n descontinuados y tengan m�s de 20 unidades en stock
Select * from Products
...
--Clientes que NO sean de M�xico o USA y que tengan fax registrado
select * from Customers
...
--Seleccionar todas las ordenes de la regi�n RJ, Tachira y que no tengan regi�n asignada
select * from Orders
...
select ProductID as N�mero, UnitPrice as Precio, ProductName as Nombre from Products
...
--Clausula Like
--Patrones:
--1) % (porcentaje) -> Representa cero o mas caracteres en el patron de b�squeda
--2) _ (gui�n bajo) -> Representa exactamente un caracter en el patron de busqueda
--3) [] (corchetes) -> Se utiliza para definir un conjunto de caracteres buscando cualquiera de ellos en la posici�n espec�fica.
--4) [^] (asento circunflejo) -> Se utiliza para buscar caracteres que no estan dentro del conjunto espec�fico
...
--Seledccionar los clientes ordenados por el pa�s y dentro por ciudad
